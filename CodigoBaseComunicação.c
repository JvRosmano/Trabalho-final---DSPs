...
void ConfiguraGpio(void)
{
    ...
    GpioDataRegs.GPASET.bit.GPIO31 = 1;  //Devido a conexão dos LEDs
...
void ConfiguraTimer(void)
{
    ...
    CpuTimer0Regs.PRD.all = 200000000*Period_Timer; //Freq do CPU * Period da interrupção
...
{
    EALLOW;
    ClkCfgRegs.PERCLKDIVSEL.bit.EPWMCLKDIV = 1; // Ver pag. 355! e é o default...

    EPwm1Regs.TBCTL.bit.HSPCLKDIV = 0; // TBCLK pre-scaler = /1


    EPwm1Regs.TBCTL.bit.CTRMODE = 2;   // Modo Up-down (simétrico)
...
void ConfiguraPWM2 (void)
{
    ...
    EPwm2Regs.TBCTL.bit.CTRMODE = 2;   // Modo Up-down (simétrico)
...
void ConfiguraPWM3 (void)
{
    ...
    EPwm3Regs.TBCTL.bit.CTRMODE = 2;   // Modo Up-down (simétrico)
...
//    V = (float)(V_teste)*1;  //(180/2048)
//
//    z = k*(V-V_linha);
//    V_linha += (z-qV_linha)*w*Period_Timer;
//    inte_a  += V_linha*Period_Timer;
//    qV_linha = w*inte_a;
//
//    // Transformada dq
...
//    omega    = Kp_pll*V_q + Ki_pll*int_srf;
//    angulo  += omega*Period_Timer;
//
//    if(angulo < 0) {angulo = 0;}
//    if(angulo > 6.283185){angulo = angulo - 6.283185;}
//
