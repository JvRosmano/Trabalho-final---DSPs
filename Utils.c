...
// Variáveis de interesse nas transformações
...
void clarkeTransform(*Clarke clarke, float a, float b, float c)
{
    clark->alfa = 0.6666666666 * (a - b * 0.5 - c * 0.5);
    clark->beta = 0.5773502691896 * (b - c);
}
void parkTransform(*Park park, Clarke clark, float angulo)
{
    cos_angulo = cos(angulo);
    sin_angulo = sin(angulo);
    park->D = (clark.alfa * cos_angulo) + (clark.beta * sin_angulo);
    park->Q = -(clark.alfa * sin_angulo) + (clark.beta * cos_angulo);
}
